...
export default function InstructorDashboard() {
  ...
  const difficultyData = ["beginner", "intermediate", "advanced"].map((d) => ({
    ...
    submissions: submissions.filter((sub) => sub.assignmentId?.difficulty === d)
      .length,
    accepted: submissions.filter(
      (sub) => sub.assignmentId?.difficulty === d && sub.status === "accepted",
    ).length,
  }));
  ...
  const stats = [
    ...
    },
    {
      ...
      value: submissions.filter((s) => s.status === "needs_improvement").length,
      icon: "🔄",
    },
  ];
